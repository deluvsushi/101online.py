...
class Client: 
    ...
    def sign(self, session_key: int):
        hash = b64encode(
            md5(
                (
                    f"{session_key}fnd;vnbwk;vb ejkwfkjew fwek fewkj fjekw; f;kao oboiboniuj"
                ).encode()
            ).digest()
        ).decode()
    ...
    def create_game(
        ...
        if password:
            ...
            data = {"fast": fast, "hand": hand, "bet": bet, "players": players, "deck": deck}
        self.send_server(data)
    ...
    def send_smile_in_game(self, smile_id: int = 18):
    ...
    def swap_position(self, id: int = 1):
        self.send_server({"command": "player_swap", "id": id})

    def send_position_swap_request(self, nickname: str, id: int = 1):
        self.send_server({"command": "player_swap_request", "id": id, "name": nickname})
